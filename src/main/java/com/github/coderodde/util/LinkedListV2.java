    ...
    implements List<E>, Deque<E>, Cloneable, java.io.Serializable
{
     ...
     * Stack of fingers.
     */
    private transient FingerList<E> fingerList = new FingerList<>();
...
//        addFinger(node, index);
        fingerList.appendFinger(node, index);
...
//            addFinger(node, index);
    ...
    ***************************************************************************/
    private void addFingersAfterSetAll() {
        ...
        fingerList.appendFinger(node, startIndex);
...
//            addFinger(node, index);
    ...
    ***************************************************************************/
    public void checkInvariant() {
        for (int i = 0, sz = fingerList.size(); i < sz; i++) {
    ...
    ***************************************************************************/
    private Finger<E> getClosestFinger(int index) {
        ...
        for (int sz = fingerList.size(), i = 0; i < sz; i++) {
            Finger<E> finger = fingerList.get(i);
    ...
    ***************************************************************************/
    private void linkLast(E e) {
        ...
        if (mustAddFinger())
            fingerList.appendFinger(newNode, size++);
    }
    ...
    ***************************************************************************/
    private static final class FingerList<E> {
        ...
        void insertFinger(Finger<E> finger) {
            ...
            final int fingerLocationIndex = 
                    getFingerLocationIndex(finger.index);
            
            System.arraycopy(
                    this.fingerArray,
                    fingerLocationIndex,
                    this.fingerArray,
                    fingerLocationIndex + 1,
                    size - fingerLocationIndex);
            
            fingerArray[fingerLocationIndex] = finger;
            size++;
        }
        
        /***********************************************************************
        Used in some addition operations in order to run in (amortized) O(1)
        time. Those operations are adding to the end of the list an element or
        a collection.
        ***********************************************************************/
        private void appendFinger(Node<E> node, int index) {
            enlargeFingerArrayIfNeeded();
            final Finger<E> finger = new Finger<>(node, index);
            this.fingerArray[size++] = finger;
        }
        
        private int getFingerLocationIndex(final int index) {
            int first = 0;
            int count = size;
            int step;
            int it;
            ...
            while (count > 0) {
                ...
                step = count / 2;
                it += step;
                if (!(index < this.fingerArray[it].index)) {
                    first = ++it;
                    count -= step + 1;
                } else {
                    count = step;
    ...
    ***************************************************************************/
    ...
    private static final class LinkedListSpliterator<E> 
        ...
        private LinkedListV2.Node<E> node;
    ...
    public static void main(String[] args) {
        ...
        list.fingerList.fingerArray = new Finger[7];
        list.fingerList.fingerArray[0] = new Finger<>(null, 1);
        list.fingerList.fingerArray[1] = new Finger<>(null, 2);
        list.fingerList.fingerArray[2] = new Finger<>(null, 5);
        list.fingerList.fingerArray[3] = new Finger<>(null, 55);
        list.fingerList.fingerArray[4] = new Finger<>(null, 55);
        list.fingerList.fingerArray[5] = new Finger<>(null, 78);
        list.fingerList.size = 6;
        
        for (int index = 0; index < 100; index++) {
            System.out.println(index + " -> " + list.fingerList.getFingerLocationIndex(index));
        }
    }
}
